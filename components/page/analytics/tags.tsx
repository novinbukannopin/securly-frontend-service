'use client';

import { TrendingUp } from 'lucide-react';
import { Label, Pie, PieChart, Sector } from 'recharts';
import { PieSectorDataItem } from 'recharts/types/polar/Pie';

import {
  Card,
  CardContent,
  CardDescription,
  CardFooter,
  CardHeader,
  CardTitle,
} from '@/components/ui/card';
import {
  ChartConfig,
  ChartContainer,
  ChartTooltip,
  ChartTooltipContent,
} from '@/components/ui/chart';
import { useGetAnalytics } from '@/service/queries/analytics';
import * as React from 'react';
import { AnalyticsResponse } from '@/types/analytics';

const chartData = [
  { browser: 'chrome', visitors: 275, fill: 'var(--color-chrome)' },
  { browser: 'safari', visitors: 200, fill: 'var(--color-safari)' },
  { browser: 'firefox', visitors: 187, fill: 'var(--color-firefox)' },
  { browser: 'edge', visitors: 173, fill: 'var(--color-edge)' },
  { browser: 'other', visitors: 90, fill: 'var(--color-other)' },
];

const chartConfig: ChartConfig = {
  marketing: {
    label: 'Marketing',
    color: 'hsl(--var(chart-1))', // Hex untuk warna oranye
  },
  social: {
    label: 'Social',
    color: '#33FF57', // Hex untuk warna hijau
  },
  promotion: {
    label: 'Promotion',
    color: '#3357FF', // Hex untuk warna biru
  },
  campaign: {
    label: 'Campaign',
    color: '#FFC300', // Hex untuk warna kuning
  },
  product: {
    label: 'Product',
    color: '#C70039', // Hex untuk warna merah gelap
  },
  email: {
    label: 'Email',
    color: '#900C3F', // Hex untuk warna magenta
  },
  advertising: {
    label: 'Advertising',
    color: '#581845', // Hex untuk warna ungu gelap
  },
  webinar: {
    label: 'Webinar',
    color: '#DAF7A6', // Hex untuk warna hijau pastel
  },
  branding: {
    label: 'Branding',
    color: '#FFC0CB', // Hex untuk warna merah muda
  },
} satisfies ChartConfig;

export function TagsChartAnalytics({
  tags,
}: {
  tags?: AnalyticsResponse['tags'];
}) {
  const response = useGetAnalytics();

  // @ts-ignore
  const data = tags?.usage.map((item) => ({
    type: chartConfig[item.tagName.toLowerCase()]?.label || item.tagName, // Gunakan label atau fallback ke type
    count: item.usageCount || 0, // Default ke 0 jika tidak ada nilai
    fill: chartConfig[item.tagName.toLowerCase()]?.color || '#CCCCCC', // Default warna
  }));

  return (
    <Card className='flex flex-col' id={'tags'}>
      <CardHeader className='items-center pb-0'>
        <CardTitle>Tags</CardTitle>
        <CardDescription>January - June 2024</CardDescription>
      </CardHeader>
      <CardContent className='flex-1 pb-0'>
        <ChartContainer
          config={chartConfig}
          className='mx-auto aspect-square max-h-[250px]'
        >
          <PieChart>
            <ChartTooltip
              cursor={false}
              content={<ChartTooltipContent hideLabel />}
            />
            <Pie
              data={data}
              dataKey='count'
              nameKey='type'
              innerRadius={60}
              strokeWidth={5}
              activeIndex={0}
              activeShape={({
                outerRadius = 0,
                ...props
              }: PieSectorDataItem) => (
                <Sector {...props} outerRadius={outerRadius + 10} />
              )}
            >
              <Label
                content={({ viewBox }) => {
                  if (viewBox && 'cx' in viewBox && 'cy' in viewBox) {
                    return (
                      <text
                        x={viewBox.cx}
                        y={viewBox.cy}
                        textAnchor='middle'
                        dominantBaseline='middle'
                      >
                        <tspan
                          x={viewBox.cx}
                          y={viewBox.cy}
                          className='fill-foreground text-3xl font-bold'
                        >
                          {data?.length}
                        </tspan>
                        <tspan
                          x={viewBox.cx}
                          y={(viewBox.cy || 0) + 24}
                          className='fill-muted-foreground'
                        >
                          Count
                        </tspan>
                      </text>
                    );
                  }
                }}
              />
            </Pie>
          </PieChart>
        </ChartContainer>
      </CardContent>
      <CardFooter className='flex-col gap-2 text-sm'>
        <div className='flex items-center gap-2 font-medium leading-none'>
          Trending up by 5.2% this month <TrendingUp className='h-4 w-4' />
        </div>
        <div className='leading-none text-muted-foreground'>
          Showing total visitors for the last 6 months
        </div>
      </CardFooter>
    </Card>
  );
}
